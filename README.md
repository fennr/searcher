...
Консольная утилита для macOS, которая подсказывает команды на русском языке через локальный сервер LM Studio/OpenAI API (`http://127.0.0.1:1234`).

## Возможности

- Проверка, что локальный API запущен (через `GET /v1/models`).
- Обычный режим: показывает несколько команд и предлагает выбрать номер.
- Режим `-r` (`--reasoning`): выдаёт текстовое объяснение и несколько подходящих команд с markdown-рендерингом в терминале.
- Флаг `--dry-run`: не выполнять выбранную команду, только показать её.
...
- Опционально для красивого рендера `-r`: `glow` или `mdcat`
...
Обычный режим (выбор команды по номеру):

```bash
searcher "как показать открытые порты"
```

Режим рассуждения:

```bash
searcher -r "как безопасно очистить кэш DNS на macOS"
```

Для корректного markdown-рендера в `-r` установите один из инструментов:
...
```bash
searcher --dry-run "как показать открытые порты"
```
...
```text
Выберите команду по номеру (Enter для отмены):
1. ...
2. ...
> 2
```
...
- `Enter` без ввода — отменить запуск
