...
- Обычный режим: выдаёт одну команду shell.
- Режим `-r` (`--reasoning`): выдаёт текстовое объяснение и несколько подходящих команд.
- Подтверждение перед финальным выводом: `y/N`.
- Флаг `--dry-run`: не выполнять команду после подтверждения, только показать её.
...
После генерации утилита показывает результат и спрашивает подтверждение:

```text
[y/N]:
```

- `y` / `yes` — подтвердить вывод
- любое другое значение — отменить
